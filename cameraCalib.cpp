...
using namespace cv;
using namespace std;
namespace fs = std::filesystem;  // C++17 标准命名空间
...
int main(int argc, char **argv) {
	// getParameters();
	string result_path="./intrinsic.txt";
	string camera_folder_path="./photos_pz/";
	int row_number=7;
	int col_number=10;
	int width=20;
	int height=20;
	...
	vector<string> filenames;
	    int valid_image_count = 0;  // 新增：有效图片计数器

	    for (const auto& entry : fs::directory_iterator(camera_folder_path)) {
	...
	// while (getline(fin, filename) && filename.size() > 1) {
		...
		valid_image_count++;  // 新增：有效图片计数器自增
		Mat view_gray;
	    ...
	    // 检查是否有足够多的有效图片
    if (valid_image_count < 5) {  // 通常至少需要5张有效图片
        cerr << "Error: Only " << valid_image_count << " valid images found. Need at least 5 for calibration." << endl;
        return -1;
    }
	...
	fout << "Overall average error is: " << total_err / valid_image_count << " pixel" << endl << endl;
