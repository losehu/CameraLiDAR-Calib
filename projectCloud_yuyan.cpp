...
cv::Mat src_img;
static float kPointAlpha = 0.60f; // 雷达点的不透明度（0=全透明，1=不透明）

int threshold_lidar = 3000;
...
static inline cv::Matx33f orthonormalize(const cv::Matx33f &Rin)
{
    ...
    for (int r = 0; r < 3; ++r)
        for (int c = 0; c < 3; ++c)
            M.at<float>(r, c) = Rin(r, c);
    cv::SVD svd(M);
    cv::Mat R = svd.u * svd.vt;
    cv::Matx33f Rout;
    for (int r = 0; r < 3; ++r)
        for (int c = 0; c < 3; ++c)
            Rout(r, c) = (float)R.at<float>(r, c);
    return Rout;
}
                     ...
                     const cv::Vec3f &tmm)
{
    ...
    putText(img, "[w/s]=+/- x  [a/d]=-/+ y  [z/x]=-/+ z   [r]=reset  [p]=print",
            Point(20, 115), FONT_HERSHEY_SIMPLEX, 0.55, Scalar(200, 200, 200), 1);
    putText(img, "[SPACE/ENTER]=next image  [q/ESC]=quit",
            Point(20, 140), FONT_HERSHEY_SIMPLEX, 0.55, Scalar(200, 200, 200), 1);
}
#include <sstream>
#include <filesystem>
...
{
    std::string img_dir ="/Users/losehu/Library/Containers/com.tencent.xinWeChat/Data/Documents/xwechat_files/wxid_y3fi4f9tcu0322_11eb/msg/file/2025-11/911_ 2/image";
    // 读取图片和PCD文件列表
    // std::string img_dir = "/Users/losehu/Library/Containers/com.tencent.xinWeChat/Data/Documents/xwechat_files/wxid_y3fi4f9tcu0322_11eb/msg/file/2025-11/911yuantu/";
    std::string pcd_dir = "/Users/losehu/Library/Containers/com.tencent.xinWeChat/Data/Documents/xwechat_files/wxid_y3fi4f9tcu0322_11eb/msg/file/2025-11/911_ 2/lidar";
    ...
    // 遍历处理
    size_t num_pairs = std::min(img_files.size(), pcd_files.size());
    std::cout << "Found " << img_files.size() << " images and " << pcd_files.size() << " PCD files\n";
    std::cout << "Processing " << num_pairs << " pairs\n";
    
    for (size_t i = 0; i < num_pairs; i++)
    {
        input_pcd_dir = pcd_files[i].string();
        input_photo_path = img_files[i].string();
        output_path = "./result.png";
        extrinsic_path = "/Users/losehu/Documents/归档/Pano_Video/calib/extrinsic_911_z_neg.txt";
        ...
        cout << "Processing image " << (i+1) << "/" << num_pairs << ": " 
             << img_files[i].filename() << " + " << pcd_files[i].filename() << "\n";
        cout << "Controls:\n"
        ...
        while (true)
        {
            ...
            Mat canvas;
            render_projection(src_img, pointcloud, dr, ext_now, threshold_lidar, canvas);
            ...
            {
                cout << "Moving to next image...\n";
                break; // 跳出当前图片的 while 循环,进入下一张
            }
